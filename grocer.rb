...
def apply_coupons(cart, coupons)
  ...
  # binding.pry
  i = 0
  while i < coupons.length && coupons.length != 0
    if cart.keys.include?(coupons[i][:item]) && cart[(coupons[i][:item])][:count] >= coupons[i][:num]
      x = (cart[(coupons[i][:item])][:count] / coupons[i][:num])
      cart[(coupons[i][:item])][:count] -= (coupons[i][:num] * x)
      cart[(coupons[i][:item]) + " W/COUPON"] = {price: coupons[i][:cost], clearance: cart[(coupons[i][:item])][:clearance], count: x}
      i+=1
    else
      i+=1
    end
